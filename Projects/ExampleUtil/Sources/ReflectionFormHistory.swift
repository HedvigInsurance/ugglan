...
public struct ReflectionFormHistory<T: Codable> {
    ...
    func storeItems(_ values: [ReflectionFormHistoryRow<T>]) {
        UserDefaults.standard.set(try? JSONEncoder().encode(values), forKey: title)
    }

    func getItems() -> [ReflectionFormHistoryRow<T>] {
        guard let prevValueJSON = UserDefaults.standard.value(forKey: title) as? Data, let decodedValue = try? JSONDecoder().decode([ReflectionFormHistoryRow<T>].self, from: prevValueJSON) else {
            return []
        }
...
extension ReflectionFormHistory: Presentable {
    public func materialize() -> (UIViewController, Signal<T>) {
        ...
        bag += tableKit.delegate.installAction(title: "Delete", style: .destructive, backgroundColor: nil, isVisibleAt: { _ -> Bool in
            true
        }).onValue { index in
        ...
        bag += tableKit.delegate.installAction(title: "Rename", style: .normal, backgroundColor: nil, isVisibleAt: { _ -> Bool in
            true
        }).onValue { index in
        ...
        bag += tableKit.delegate.installAction(title: "Edit", style: .normal, backgroundColor: nil, isVisibleAt: { _ -> Bool in
            true
        }).onValue { index in
            var item = tableKit.table[index]
