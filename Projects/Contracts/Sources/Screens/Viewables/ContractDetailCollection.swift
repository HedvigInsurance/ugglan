...
extension ContractDetailCollection: Viewable {
    func materialize(events _: ViewableEvents) -> (UICollectionView, Disposable) {
        ...
        bag += $rows.atOnce().map { Table(rows: $0) }.onValue { table in
            collectionKit.set(table)
        }

        bag += $currentIndex.onValue { index in
            collectionKit.view.setContentOffset(
                CGPoint(x: CGFloat(index.row) * collectionKit.view.frame.width, y: 0),
                animated: true
            )
        }
        ...
        bag += collectionKit.delegate.sizeForItemAt.set { index -> CGSize in
            ...
            return CGSize(
                width: collectionKit.view.frame.width,
                height: row.calculateContentSize(collectionKit.view.frame.size).height
            )
        }
        ...
        bag += collectionKit.delegate.willDisplayCell.onValue { _, index in
            ...
            let height = row.calculateContentSize(collectionKit.view.frame.size).height
