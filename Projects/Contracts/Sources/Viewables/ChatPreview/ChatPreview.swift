...
extension GraphQL.ChatPreviewSubscriptionSubscription.Data.Message: Equatable {
    public static func == (lhs: GraphQL.ChatPreviewSubscriptionSubscription.Data.Message, rhs: GraphQL.ChatPreviewSubscriptionSubscription.Data.Message) -> Bool {
...
extension ChatPreview: Viewable {
    func materialize(events _: ViewableEvents) -> (UIView, Disposable) {
        ...
        let symbolIcon = Icon(icon: hCoreUIAssets.symbol.image, iconWidth: 20)
        ...
        bag += containerView.addArranged(Divider(
            backgroundColor: .brand(.primaryBorderColor)
        ))
...
//        func getMessagesToShow(messages: [GraphQL.ChatPreviewQuery.Data.Message]) -> Signal<[GraphQL.ChatPreviewQuery.Data.Message]> {
// todo
...
//        func loadData() {
//            bag += client.fetch(query: GraphQL.ChatPreviewQuery(), cachePolicy: .fetchIgnoringCacheData).valueSignal
...
//        }
        ...
        func setupSubscription() {
            subscriptionBag += client.subscribe(
                subscription: GraphQL.ChatPreviewSubscriptionSubscription()
            )
