...
extension ContractRow {
	var isContractActivated: Bool {
		contract.status.asActiveStatus != nil || contract.status.asTerminatedTodayStatus != nil
			|| contract.status.asTerminatedInFutureStatus != nil
	}
	...
	var statusPills: [String] {
		if let status = contract.status.asActiveInFutureAndTerminatedInFutureStatus {
			...
			return [L10n.dashboardInsuranceStatusInactiveStartdate(futureInceptionDate.localized)]
		} else if contract.status.asPendingStatus != nil {
			return [L10n.dashboardInsuranceStatusInactiveNoStartdate]
		} else if contract.status.asTerminatedStatus != nil {
			return [L10n.dashboardInsuranceStatusTerminated]
		}
	...
	private var coversHowManyPill: String {
		func getPill(numberCoinsured: Int) -> String {
			numberCoinsured > 0
				? L10n.InsuranceTab.coversYouPlusTag(numberCoinsured) : L10n.InsuranceTab.coversYouTag
		}

		switch type {
		case .swedishApartment:
			let numberCoinsured =
				contract.currentAgreement.asSwedishApartmentAgreement?.numberCoInsured ?? 0
		...
		case .norwegianHome:
			let numberCoinsured =
				contract.currentAgreement.asNorwegianHomeContentAgreement?.numberCoInsured ?? 0
			return getPill(numberCoinsured: numberCoinsured)
		...
		case .danishHome:
			let numberCoinsured =
				contract.currentAgreement.asDanishHomeContentAgreement?.numberCoInsured ?? 0
			return getPill(numberCoinsured: numberCoinsured)
		case .danishTravel:
