...
class AddressState {
    ...
    func getSuggestions(searchTerm: String, suggestion: AddressSuggestion?) -> Future<[AddressSuggestion]> {
        ...
        return self.client.fetch(query: GraphQL.AddressAutocompleteQuery(input: queryParams.apiQuery, type: queryParams.searchType)).map { $0.autoCompleteAddress }
	}
    
    func getApiQueryParams(_ searchTerm: String, _ suggestion: AddressSuggestion?) -> (apiQuery: String, searchType: SearchType) {
        if let suggestion = suggestion {
            ...
            if let city = suggestion.city, let postalCode = suggestion.postalCode, isMatchingStreetName(searchTerm, suggestion) {
