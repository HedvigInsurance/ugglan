...
extension Action: Viewable {
	func materialize(events _: ViewableEvents) -> (UIView, Signal<GraphQL.EmbarkLinkFragment>) {
		...
		bag += view.didLayoutSignal.withLatestFrom(isHiddenSignal.atOnce().plain()).map { _, isHidden in
			isHidden
		}.onValue(handleViewState)
		...
		let hideAnimationSignal = actionDataSignal.withLatestFrom(state.passageNameSignal).animated(
			style: animationStyle
		) { _, _ in isHiddenSignal.value = true
		...
		return (
			...
			Signal { callback in
				...
				bag += actionDataSignal.withLatestFrom(self.state.passageNameSignal).wait(
					until: shouldUpdateUISignal
					...
					if let selectAction = actionData?.asEmbarkSelectAction {
						innerBag += view.addArranged(
							EmbarkSelectAction(state: self.state, data: selectAction)
						).onValue(performCallback)
					} else if let textAction = actionData?.asEmbarkTextAction {
						innerBag += view.addArranged(
							EmbarkTextAction(state: self.state, data: textAction)
						).onValue(performCallback)
					} else if let numberAction = actionData?.asEmbarkNumberAction {
						innerBag += view.addArranged(
							EmbarkNumberAction(state: self.state, data: numberAction)
						).onValue(performCallback)
