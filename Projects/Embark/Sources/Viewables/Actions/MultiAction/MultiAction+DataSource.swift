...
struct MultiActionDataSource {
    internal init(maxCount: Int, addLabelTitle: String) {
    ...
    func addValue(values: [String: MultiActionValue]) {
        let title = values.first(where: { (key, _) -> Bool in
            key == "type"
        })?.value.displayValue
    ...
    func lazyLoadAddObjectRow() {
        $rows.value = [addObjectRow]
    }
}

private extension Array where Element == MultiActionRow {
