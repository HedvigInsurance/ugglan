...
typealias EmbarkNumberActionSetData = GraphQL.EmbarkStoryQuery.Data.EmbarkStory.Passage.Action.AsEmbarkNumberActionSet.Datum

typealias EmbarkTextActionSetData = GraphQL.EmbarkStoryQuery.Data.EmbarkStory.Passage.Action.AsEmbarkTextActionSet.TextActionSetDatum
...
struct EmbarkActionSetInputData {
    internal init(numberActionSet: EmbarkNumberActionSetData, state: EmbarkState) {
        actions = numberActionSet.numberActions.map { numberAction in
            .init(placeholder: numberAction.data?.placeholder,
    ...
    internal init(textActionSet: EmbarkTextActionSetData, state: EmbarkState) {
        actions = textActionSet.textActions.map { textAction in
            .init(placeholder: textAction.data?.placeholder,
...
extension EmbarkActionSetInputData: Viewable {
    func materialize(events _: ViewableEvents) -> (UIView, Signal<GraphQL.EmbarkLinkFragment>) {
        ...
        func getMasking(_ action: Action) -> Masking? {
            guard let mask = action.mask else {
                return nil
            }
        ...
        return (view, Signal { callback in
            func complete() {
                ...
                if let passageName = self.state.passageNameSignal.value {
                    self.state.store.setValue(
                        ...
                        value: actionSignals.map { $0.signal.value }.joined(separator: " ")
                    )
                }
            ...
            bag += actionSignals.map { _, shouldReturn, _ in shouldReturn }.enumerated().map { offset, shouldReturn in
                shouldReturn.set { value -> Bool in
                    if !value.isEmpty {
                        if offset == actionSignals.count - 1 {
                            complete()
                        }
            ...
            bag += combineLatest(actionSignals.map { signal, _, action in isValid(signal: signal, action: action) })
                .map {
                    !$0.contains(false)
