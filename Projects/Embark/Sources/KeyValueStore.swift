...
public struct EmbarkStoryState {
    let kvs: KeyValueStore
    let passageHistory: [EmbarkPassage]
    let currentPassage: EmbarkPassage
}

public struct EmbarkActionState: StateProtocol {
    var currentStory: EmbarkStoryState
...
public final class EmbarkStateStore: StateStore<EmbarkActionState, EmbarkActions> {

    public override func effects(_ getState: () -> EmbarkActionState, _ action: EmbarkActions) -> FiniteSignal<EmbarkActions>? {
        
        return nil
    }

    public override func reduce(_ state: EmbarkActionState, _ action: EmbarkActions) -> EmbarkActionState {
        var newState = state

        switch action {
        
        case .restart:
            newState.embarkValues = .init()
        case .removeRevision:
            newState.embarkValues.removeLastRevision()
        case .createRevision:
            newState.embarkValues.createRevision()
        case .setComputeValues(values: let values):
            newState.embarkValues.computedValues = values
        case .setValue(let key, let value):
            newState.embarkValues.setValue(key: key, value: value)
        }
        return newState
    }
}
