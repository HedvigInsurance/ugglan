...
struct CompareTierScreen: View {
    ...
    init(
        ...
        self.limits = Dictionary(
            uniqueKeysWithValues: vm.tiers.map({ ($0.id, $0.deductibles.first?.productVariant?.insurableLimits ?? []) })
        )
...
extension ChangeTierViewModel {
    fileprivate func getFilteredPerils(currentTier: Tier) -> [Perils] {
        var currentPerils = currentTier.deductibles.first?.productVariant?.perils ?? []
        let otherPerils = self.tiers.filter({ $0.id != currentTier.id })
            .reduce(into: [Perils]()) { partialResult, tier in
                return partialResult.append(contentsOf: tier.deductibles.first?.productVariant?.perils ?? [])
            }
