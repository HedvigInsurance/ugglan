...
public struct BankIDLoginSweden: View {
            ...
            VStack(spacing: 16) {
                ...
                VStack {
                    PresentableStoreLens(
                        ...
                        getter: { state in
                            state.statusText ?? "Väntar på bankID..."
                        }
                    ) { statusText in
                        hText(statusText)
                            .foregroundColor(hTextColorNew.primaryTranslucent)
                    }
                    hText("Öppna BankID på din telefon för att teckna din försäkring")
                        .multilineTextAlignment(.center)
                        .foregroundColor(hTextColorNew.secondaryTranslucent)
                }
                Image(uiImage: HCoreUIAsset.menuIcon.image)
                    .padding(.top, 16)
            }
