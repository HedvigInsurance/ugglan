...
public struct ShapeEnvironmentRedraw<Content: View>: View {
    ...
    init(@ViewBuilder _ content: @escaping (_ colorScheme: ColorScheme, _ userInterfaceLevel: UIUserInterfaceLevel) -> Content) {
...
public struct hColorScheme<LightInnerHColor: hColor, DarkInnerHColor: hColor>: hColor {
    ...
    init(light: Color, dark: Color) where LightInnerHColor == hColorBase, DarkInnerHColor == hColorBase {
        self.light = hColorBase(light)
        self.dark = hColorBase(dark)
    }

    init(light: LightInnerHColor, dark: DarkInnerHColor) {
        self.light = light
        self.dark = dark
    }
    
    init(_ always: Color) where LightInnerHColor == hColorBase, DarkInnerHColor == hColorBase {
...
public struct hColorLevel<InnerHColor: hColor>: hColor {
    ...
    init(base: Color, elevated: Color) where InnerHColor == hColorBase {
        self.base = hColorBase(base)
        self.elevated = hColorBase(elevated)
    }
    
    init(base: InnerHColor, elevated: InnerHColor) {
        self.base = base
        self.elevated = elevated
    }
    
    init(_ always: Color) where InnerHColor == hColorBase {
