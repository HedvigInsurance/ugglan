    ...
    struct OverlayModifier: ViewModifier {
        ...
        func body(content: Content) -> some View {
            if colorScheme == .light {
                content.overlay(
                    RoundedRectangle(cornerRadius: .defaultCornerRadius).stroke(hLabelColor.primary, lineWidth: configuration.isPressed ? 0 : 1)
                )
            } else {
                content.overlay(
                    RoundedRectangle(cornerRadius: .defaultCornerRadius).stroke(hLabelColor.primary, lineWidth: 1)
                )
            }
        }
    }
