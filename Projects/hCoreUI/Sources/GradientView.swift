...
public struct GradientView: View {
    ...
    public var body: some View {
        GradientViewRepresentable()
    }
}

struct GradientViewRepresentable: UIViewRepresentable {
    @ReadWriteState public var gradientOption: GradientView.GradientOption?
    @ReadWriteState public var shouldShowGradient = false

    func makeUIView(context: Context) -> some UIView {
        CustomGradientView(gradientOption: gradientOption, shouldShowGradient: shouldShowGradient)
    }
...
private class CustomGradientView: UIView {
    @ReadWriteState public var gradientOption: GradientView.GradientOption?
    @ReadWriteState public var shouldShowGradient = false
    ...
    init(
        ...
        bag += gradientView.didMoveToWindowSignal.onValue({ _ in
            let signal = self.$gradientOption.atOnce().filter(predicate: { $0 != nil }).distinct()

            bag += signal.onFirstValue({ gradientOption in
                self.applySettings(layer, gradientView.traitCollection)
            })

            bag += signal.skip(first: 1)
                .onValue({ gradientOption in
                    self.applySettingsWithAnimation(layer, gradientView.traitCollection)
                })
        })
        ...
        bag += gradientView.traitCollectionSignal
            ...
            .onValue({ traitCollection in
                self.applySettings(layer, traitCollection)
            })
