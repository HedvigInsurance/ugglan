...
extension InsurableLimits: Viewable {
    public func materialize(events _: ViewableEvents) -> (UIView, Disposable) {
        ...
        let section = SectionView(headerView: UILabel(value: L10n.contractCoverageMoreInfo, style: .default), footerView: nil)
        
        bag += insurableLimitFragmentsSignal.atOnce().onValueDisposePrevious { insurableLimitFragments in
            ...
            innerBag += insurableLimitFragments.map { insurableLimitFragment in
                ...
                row.append(UILabel(value: insurableLimitFragment.limit, style: .brand(.body(color: .secondary))))
