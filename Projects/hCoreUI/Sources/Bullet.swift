...
extension Bullet: Viewable {
    ...
    public func materialize(events: ViewableEvents) -> (UIControl, Disposable) {
        ...
        bag += $isSelected.atOnce().withLatestFrom(control.traitCollectionSignal).animated(style: SpringAnimationStyle.lightBounce(), animations: { (isSelected, traitCollection) in
