            ...
            .onValueDisposePrevious { toast in
                ...
                if let previousToast = containerView.subviews.last {
                    bag += Signal(after: 0).animated(style: .lightBounce(duration: 1)) {
                        previousToast.transform = previousToast.transform.concatenating(CGAffineTransform(scaleX: 0.5, y: 0.5))
                    }

                    bag += Signal.animatedDelay(after: 0.5).animated(style: .lightBounce(duration: 0.25)) {
                ...
                bag += containerView.add(toast) { toastView in
                    ...
                    innerBag += panGestureRecognizer.signal(forState: .changed).onValue {
                        ...
                        toastView.transform = CGAffineTransform(translationX: 0, y: translationY)
                    }

                    innerBag += Signal(after: 0).feedback(type: .impactLight)

                    innerBag += Signal(after: 0).animated(style: SpringAnimationStyle.lightBounce(delay: 0, duration: 1)) { _ in
                    ...
                    bag += hideCallbacker.atValue { _ in
                        pauseSignal.value = true
                    }
                    .throttle(1)
                    ...
                    innerBag += panGestureRecognizer.signal(forState: .ended).onValue {
                        ...
                        if location.y < -20 || velocity.y > 1300 {
                            ...
                            innerBag += Signal(after: 0).animated(style: .lightBounce()) { _ in
                                toastView.transform = CGAffineTransform.identity
                            }.onValue { _ in
                                pauseSignal.value = false
                            }
                    ...
                    innerBag += toast.shouldHideCallbacker.atValue {
                        pauseSignal.value = true
                    }.delay(by: 0.25).onValue {
                        hideCallbacker.callAll()
                    }
