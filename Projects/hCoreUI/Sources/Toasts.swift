...
extension Toast: Viewable {
	...
	public func materialize(events _: ViewableEvents) -> (UIView, Disposable) {
		...
		bag += containerView.signal(for: .touchUpInside).atValue {
		...
		bag += containerView.delayedTouchCancel(delay: 0.1).animated(style: SpringAnimationStyle.lightBounce())
		{ _ in if !self.onTapCallbacker.isEmpty { containerView.transform = CGAffineTransform.identity } }
		...
		bag += toastCallbacker.compactMap { $0 }.wait(until: pauseSignal.distinct().map { !$0 }).distinct()
			.onValueDisposePrevious { toast in let innerBag = bag.innerBag()
				...
				if let previousToast = containerView.subviews.last {
					bag += Signal(after: 0).animated(style: .lightBounce(duration: 1)) {
						previousToast.transform = previousToast.transform.concatenating(
							CGAffineTransform(scaleX: 0.5, y: 0.5)
						)
					}

					bag += Signal.animatedDelay(after: 0.5).animated(
						style: .lightBounce(duration: 0.25)
					) { previousToast.alpha = 0 }.onValue { _ in previousToast.removeFromSuperview()
					}
				}
				...
				bag += containerView.add(toast) { toastView in
					...
					) { _ in toastView.transform = CGAffineTransform.identity }.onValue { _ in
					...
					bag += hideCallbacker.atValue { _ in pauseSignal.value = true }.throttle(1)
						.filter(predicate: { containerView.subviews.count == 1 }).animated(
							style: .lightBounce(duration: 1)
						) { _ in
							toastView.transform = CGAffineTransform(
								translationX: 0,
								y: -125
					...
					hideBag += Signal.animatedDelay(after: toast.duration).onValue { _ in
					...
					innerBag += panGestureRecognizer.signal(forState: .ended).onValue {
						...
						if location.y < -20 || velocity.y > 1300 {
							...
							innerBag += Signal(after: 0).animated(style: .lightBounce()) {
					...
					innerBag += pauseSignal.distinct().onValue { pause in
						if pause {
							...
							hideBag += Signal.animatedDelay(after: toast.duration).onValue {
								_ in hideCallbacker.callAll()
							}
						}
					}
				}
