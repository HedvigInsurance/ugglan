...
- Key services and protocols:
  ...
  - `hClaimFileUploadClient` (protocol) -- uploads files to a claim endpoint with progress reporting. Wrapped by `hClaimFileUploadService` (logging facade at `Service/OctopusImplementation/UploadClaimFileClientOctopus.swift`). Octopus implementation lives in `Projects/App/Sources/Service/OctopusClientsImplementation/SubmitClaim/SubmitClaimFileUploadOctopus.swift`. Demo in `Service/DemoImplementation/UploadClaimFileDemoClient.swift`.
...
## Gotchas
- **Local service wrapper**: `Sources/Service/OctopusImplementation/UploadClaimFileClientOctopus.swift` contains `hClaimFileUploadService` (a logging wrapper), not the actual Octopus implementation. The real `hClaimFileUploadClient` Octopus implementation lives in `Projects/App/Sources/Service/OctopusClientsImplementation/SubmitClaim/SubmitClaimFileUploadOctopus.swift`. The other two Octopus implementations (`FetchClaimsClientOctopus`, `FetchClaimDetailsClientOctopus`) also live in the App module.
