...
extension RenewalCard: Viewable {
	func materialize(events _: ViewableEvents) -> (UIStackView, Disposable) {
		...
		bag += client.watch(query: GraphQL.HomeQuery()).map {
			$0.contracts.filter { contract in contract.upcomingRenewal != nil }
		}.filter { !$0.isEmpty }.onValueDisposePrevious { contracts -> Disposable? in let bag = DisposeBag()
				...
				}), let renewalDate = contracts.first?.upcomingRenewal?.renewalDate.localDateToDate
			{
				...
				bag += stackView.addArranged(
					...
					guard let viewController = buttonView.viewController else { return }
					...
					let alert = Alert(
						actions: [
							...
							[
								Alert.Action(
									title: L10n.DashboardMultipleRenewalsPrompter
										.ActionSheet.cancel,
									style: .cancel,
									action: {}
								)
					...
					viewController.present(alert, style: .sheet(from: buttonView, rect: nil))
				}
			} else {
