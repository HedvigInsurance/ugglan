...
//        self._vm = .init(initialValue: .init(helpCenterModel: helpCenterModel))
    }
...
class HelpCenterStartViewModel: ObservableObject {
    ...
    @Published var searchResults: [Question]?
    ...
    func startSearch() {
        searchResults = if (!inputText.isEmpty) {
             searchInQuestionsByQuery(query: inputText)
        } else {
            nil
        }
    }
    ...
    private func searchInQuestionsByQuery(query: String) -> [Question] {
        ...
        let trimmedQuery: String = query.lowercased().trimmingCharacters(in: .whitespaces)
        allQuestions.forEach{ question in
            if (question.answer.lowercased().contains(trimmedQuery)  || question.question.lowercased().contains(trimmedQuery)){
