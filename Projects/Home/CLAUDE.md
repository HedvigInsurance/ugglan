...
## Architecture
- **Pattern**: `HomeStore` is an `@PersistableStore`-backed `AppStore` (`AppStateContainer`) with `@Published` properties for member contract state, contracts, important messages, quick actions, FAQ, and chat-notification state. `HomeScreen` reads it via `@AppObservedObject` and renders `ActiveHomeView` for every member state; a thin `HomeVM` drives the cross-store fetches. `HomeBottomScrollView` has its own `HomeBottomScrollViewModel` for composing the bottom-card list, which includes the one contract-status card (terminated, or one of the three `FutureStatus` messages).
...
- **Redesigned layout**: `Screens/ActiveHomeView.swift` -- the hero + greeting + sheet layout every member state renders
- **Components**: `Screens/Components/GreetingView.swift`, `HomeSheetContainer.swift`, `HomeActionChips.swift`, `TodoList.swift`, `ImportantMessagesView.swift`, `RenewalCard.swift`, `HomeQuickActionsSection.swift`, `HomeOngoingQuotesSection.swift`, `HomeCrossSellsSection.swift`, `HomeAddonsSection.swift`
...
- **Models**: `Models/Contract.swift`, `ImportantMessage.swift`, `QuickAction.swift`, `HomeQuickAction.swift` (Home tile vocabulary; eligibility and order live in `HomeStore.init()`), `HelpCenterFAQModel.swift`, `MemberContractState.swift`, `UpcomingRenewal.swift`
...
## Gotchas
- `HomeVM` holds no state of its own; it triggers the cross-store fetches in `fetchHomeState()` for Home/CrossSell/Contract/Payment stores and owns the two polling timers.
...
- `HomeStore` snapshot decode is all-or-nothing: removing a persisted property or adding an `Optional` one is safe; a new non-Optional property or a changed type wipes every member's restored `HomeStore`. `homeQuickActions` is therefore `@Transient`; `quickActions` stays persisted because `toolbarOptionTypes` reads `quickActions.hasFirstVet`.
- `.upgradeCoverage` only ever arrives nested inside `.editInsurance(actions:)`, never top-level -- the pipeline unwraps it from `EditInsuranceActionsWrapper`.
- Invite a friend and Upcoming payment are Home detents (`HomeNavigationViewModel.isForeverPresented` / `isUpcomingPaymentPresented`, hosted in `HomeTab`), not `QuickActionsViewModel` actions; the Upcoming payment tile carries its `PaymentData`.
- Ongoing quotes (unfinished web shop sessions) are gated in the data layer, not the view: `HomeStore.fetchOngoingQuotes()` checks `isResumingOngoingShopSessionsEnabled` and leaves `ongoingQuotes` empty when the flag is off, so `HomeOngoingQuotesSection` self-hides. The flag is the inverted Unleash kill switch `disable_resuming_ongoing_shop_sessions`, matching Android. `ongoingQuotes` is `@Transient` because a quote can be signed or expire on the web while the app is closed; the backend already excludes signed and expired sessions, so there is no client-side state or expiry filtering. Each card resumes by handing `resumeUrl` (auth-free) to the device browser via `Dependencies.urlOpener`.
