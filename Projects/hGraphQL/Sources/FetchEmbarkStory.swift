...
extension ApolloClient {
    public func fetchEmbarkStory(name: String, locale: String) -> Future<hEmbarkStory> {
        Future<hEmbarkStory> { completion in
            let cancellable = self.fetch(query:
...
public struct hEmbarkStory: Codable {
    ...
    public init(story: GraphQL.EmbarkStoryQuery.Data.EmbarkStory) {
        self.id = story.id
        self.passages = story.passages.map { .init(passage: $0) }
        self.initialPassage = story.passages.first(where: { $0.name == story.startPassage }).map { .init(passage: $0) }
    }
}
...
public struct hEmbarkPassage: Codable {
    ...
    init(passage: GraphQL.EmbarkStoryQuery.Data.EmbarkStory.Passage) {
        name = passage.name
