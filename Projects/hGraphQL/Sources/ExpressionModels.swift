...
public struct Expression: Codable {
    internal init(type: ExpressionType, typename: ExpressionTypename, key: String? = nil, value: String? = nil, text: String? = nil, passedExpressionKey: String? = nil, passedExpressionValue: String? = nil, subExpressions: [SubExpression]? = nil, to: String? = nil) {
        self.type = type
...
public struct SubExpression: Codable {
    ...
    public init?(subExpression: GraphQL.BasicExpressionFragment) {
    ...
    public init?(subExpression: GraphQL.ExpressionFragment) {
        if let expression = subExpression.asEmbarkExpressionUnary {
            self.key = nil
            self.value = nil
            self.text = expression.text
            ...
            self.subExpressions = expression.subExpressions.compactMap {
                if let subExpression = $0.asEmbarkExpressionMultiple {
                    return SubExpression.init(subExpression: subExpression.fragments.basicExpressionFragment)
                } else { return nil }
            }
        } else { return nil }
    }
...
public struct hEmbarkRedirect: Codable {
    ...
    public init(redirect: GraphQL.EmbarkStoryQuery.Data.EmbarkStory.Passage.Redirect) {
        if let unary = redirect.asEmbarkRedirectUnaryExpression, unary.unaryType == .always {
            expression = .init(type: .always, typename: .unary, key: nil, value: nil, text: nil, passedExpressionKey: unary.passedExpressionKey, passedExpressionValue: unary.passedExpressionValue, subExpressions: nil, to: unary.to)
        } else if let binary = redirect.asEmbarkRedirectBinaryExpression {
            expression = .init(type: (.init(rawValue: binary.binaryType.rawValue) ?? .and), typename: .binary, key: binary.key, value: binary.value, text: nil, passedExpressionKey: binary.passedExpressionKey, passedExpressionValue: binary.passedExpressionValue, subExpressions: nil)
        } else if let multiple = redirect.asEmbarkRedirectMultipleExpressions {
            expression = .init(type: .init(rawValue: multiple.multipleType.rawValue) ?? .and, typename: .multiple, key: nil, value: nil, text: nil, passedExpressionKey: multiple.passedExpressionKey, passedExpressionValue: multiple.passedExpressionValue, subExpressions: multiple.subExpressions.compactMap {
                if let subExpression = $0.asEmbarkExpressionMultiple {
                    return SubExpression.init(subExpression: subExpression.fragments.basicExpressionFragment)
                } else { return nil }
            })
