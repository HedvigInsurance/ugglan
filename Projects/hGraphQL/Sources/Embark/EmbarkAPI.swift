...
public struct hAPI: Codable {
    ...
    init?(api: GraphQL.EmbarkStoryQuery.Data.EmbarkStory.Passage.Action.AsEmbarkSelectAction.SelectActionDatum.Option.Api?) {
        ...
        if let api = api.asEmbarkApiGraphQlQuery {
            ...
            data = .init(next: next, query: api.data.query, variables: api.data.variables.compactMap { .init(variable: $0) }, results: api.data.queryResults.map { .init(result: $0) }, errors: api.data.queryErrors.map { .init(error: $0) })
        } else if let api = api.asEmbarkApiGraphQlMutation {
            type = .graphQLMutation
            let next = api.data.next.map { hEmbarkLink(name: $0.name, label: $0.label) }
            data = .init(next: next, query: api.data.mutation, variables: api.data.variables.compactMap { .init(variable: $0) }, results: [], errors: [])
        } else {
            return nil
        }
    }
    ...
    public struct Variables: Codable {
        ...
        init?(variable: GraphQL.EmbarkStoryQuery.Data.EmbarkStory.Passage.Action.AsEmbarkSelectAction.SelectActionDatum.Option.Api.AsEmbarkApiGraphQlQuery.Datum.Variable) {
        ...
        init?(variable: GraphQL.ApiVariablesFragment) {
            if let variable = variable.asEmbarkApiGraphQlGeneratedVariable {
                self.init(generatedVariable: variable.fragments.apiGeneratedVariableFragment)
            } else if let variable = variable.asEmbarkApiGraphQlSingleVariable {
                self.init(singleVariable: variable.fragments.apiSingleVariableFragment)
        ...
        init?(variable: GraphQL.EmbarkStoryQuery.Data.EmbarkStory.Passage.Action.AsEmbarkSelectAction.SelectActionDatum.Option.Api.AsEmbarkApiGraphQlMutation.Datum.Variable) {
            if let variable = variable.asEmbarkApiGraphQlGeneratedVariable {
                self.init(generatedVariable: variable.fragments.apiGeneratedVariableFragment)
            } else if let variable = variable.asEmbarkApiGraphQlSingleVariable {
                self.init(singleVariable: variable.fragments.apiSingleVariableFragment)
        ...
        init(generatedVariable: GraphQL.ApiGeneratedVariableFragment) {
            type = .generated
        ...
        init(multiVariable: GraphQL.ApiMultiActionVariableFragment) {
            type = .generated
            key = multiVariable.key
            from = nil
            self.as = nil
            let generatedVariables = multiVariable.variables.compactMap {
            ...
            let singleVariables = multiVariable.variables.compactMap {
                if let fragment = $0.fragments.apiGeneratedVariableFragment {
                    return fragment
                } else { return nil }
            }.map {
        ...
        init(singleVariable: GraphQL.ApiSingleVariableFragment) {
            type = .generated
    ...
    public struct QueryResult: Codable {
        public let key: String?
        public let `as`: String?
        init(result: GraphQL.EmbarkStoryQuery.Data.EmbarkStory.Passage.Action.AsEmbarkSelectAction.SelectActionDatum.Option.Api.AsEmbarkApiGraphQlQuery.Datum.QueryResult) {
            self.key = result.key
            self.as = result.as
        }
    }
    ...
    public struct QueryError: Codable {
        ...
        public let next: hEmbarkLink?
        init(error: GraphQL.EmbarkStoryQuery.Data.EmbarkStory.Passage.Action.AsEmbarkSelectAction.SelectActionDatum.Option.Api.AsEmbarkApiGraphQlQuery.Datum.QueryError) {
