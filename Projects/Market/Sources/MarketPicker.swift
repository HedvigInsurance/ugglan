...
public struct MarketPicker {
	...
	public init(didFinish: @escaping () -> Void = {}) { self.didFinish = didFinish }
}
		...
		bag += client.fetch(query: GraphQL.MarketQuery()).valueSignal.onValue { data in
			...
			bag += form.append(
				continueButton.insetted(UIEdgeInsets(horizontalInset: 15, verticalInset: 0)) {
					...
					bag += localeUpdatedSignal.atOnce().transition(
			...
			bag += ApplicationContext.shared.$hasFinishedBootstrapping.atOnce().delay(by: 1.25).take(
				first: 1
			).animated(
