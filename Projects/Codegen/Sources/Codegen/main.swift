...
func getFolders(basePath: String = sourceRootURL.path, accessModifier: ApolloCodegenOptions.AccessModifier) -> [URL] {
	...
	let ownDirs = dirs.filter { $0 == accessModifier.directory }.map { URL(string: "file://\(basePath)/\($0)") }
		.compactMap { $0 }
...
func findAndAddGraphQLFolders(for folderURLS: [URL], accessModifier: ApolloCodegenOptions.AccessModifier) {
	folderURLS.forEach { sourceUrl in
		...
		let fromUrl =
			isGraphQLFolder
			? sourceUrl.appendingPathComponent("../").appendingPathComponent("../") : sourceUrl
		...
		if !isGraphQLFolder {
			...
			allGeneratedFiles.filter { generatedFile in
				...
				return allGraphQLFiles.first(where: { $0.lastPathComponent.contains(originalFileName) })
					== nil
			}
		...
		if accessModifier == .internal {
			...
			let allGeneratedFiles = try! FileManager.default
				.contentsOfDirectory(
					at: folderUrl,
					includingPropertiesForKeys: nil,
					options: []
				...
				.filter { generatedFile in
					...
					return internalGraphQLFiles.first(where: {
