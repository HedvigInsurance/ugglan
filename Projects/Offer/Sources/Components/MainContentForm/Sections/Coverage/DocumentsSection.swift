...
extension DocumentsSection: Presentable {
    func materialize() -> (SectionView, Disposable) {
        ...
        bag += quote.insuranceTerms.map { term in
            ...
            innerBag += rowControl.signal(for: .touchDown).animated(style: .easeOut(duration: 0.25), animations: { _ in
                rowControl.backgroundColor = backgroundColor.darkened(amount: 0.05)
            })

            innerBag += rowControl.delayedTouchCancel().animated(style: .easeOut(duration: 0.25), animations: { _ in
                rowControl.backgroundColor = backgroundColor
            })
            ...
            rowContentContainer.addArrangedSubview(hCoreUIAssets.document.image.imageView(height: 34, width: 34))
            ...
            innerBag += rowControl.signal(for: .touchUpInside).onValue { _ in
