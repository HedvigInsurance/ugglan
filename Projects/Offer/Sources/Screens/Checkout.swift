...
struct CheckoutButton: Presentable {
    ...
    func materialize() -> (UIView, Disposable) {
        ...
        bag += view.keyboardSignal(priority: .contentInsets).onValue({ keyboard in
            ...
            containerView.layoutMargins = baseLayoutMargins + UIEdgeInsets(
                top: 0,
                left: halfWidth,
...
extension Checkout: Presentable {
    func materialize() -> (UIViewController, Future<Void>) {
        ...
        bag += state.dataSignal.compactMap { $0.quoteBundle }.onFirstValue({ quoteBundle in
            let header = UIStackView()
            header.spacing = 16
            header.axis = .vertical
            
            let titleLabel = MultilineLabel(value: quoteBundle.quotes.reduce("", { previousString, quote in
                return previousString.isEmpty ? quote.displayName : "\(previousString) + \n\(quote.displayName)"
            }), style: .brand(.title1(color: .secondary)).restyled({ (style: inout TextStyle) in
            ...
            let ssnRow = RowView(title: "National Identity Number", style: .brand(.title3(color: .primary)))
            ...
            bag += isValidSignal.filter { valid in valid }.onValue { _ in
                checkoutButton.$isLoading.value = true
                
                join(quoteBundle.quotes.map { quote in state.checkoutUpdate(
                    quoteId: quote.id,
                    email: emailMasking.unmaskedValue(text: emailTextField.value),
                    ssn: ssnMasking.unmaskedValue(text: ssnTextField.value)
                ) }).onValue { _ in
