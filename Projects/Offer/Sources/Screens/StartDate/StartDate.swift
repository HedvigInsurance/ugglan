	...
	func materialize() -> (UIViewController, Future<Void>) {
        ...
        bag += state.quotesSignal.onValueDisposePrevious { quotes in
            ...
            bag += quotes.map { quote in
                form.append(SingleStartDateSection(
        ...
        let saveButton = Button(title: "Save", type: .standard(backgroundColor: .brand(.secondaryButtonBackgroundColor), textColor: .brand(.secondaryButtonTextColor)))
        ...
        return (viewController, Future { completion in
            bag += loadableSaveButton.onTapSignal.onValue { _ in
                ...
                join(selectedDatesMap.map { quoteId, date in
