...
extension SingleStartDateSection: Presentable {
	func materialize() -> (SectionView, ReadWriteSignal<Date?>) {
        ...
        bag += dateSignal.atOnce().onValue { date in
        ...
        bag += picker.distinct().onValue { date in
        ...
        if isCollapsible {
            bag += dateSignal.atOnce().animated(style: .easeOut(duration: 0.25)) { date in
                if date == nil {
                    row.alpha = 0.5
                } else {
                    row.alpha = 1
        ...
        bag += switcherSwitch.distinct().onValue { active in
            if active {
