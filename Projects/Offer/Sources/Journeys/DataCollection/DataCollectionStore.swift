...
public final class DataCollectionStore: StateStore<DataCollectionState, DataCollectionAction> {
    ...
    func dataCollectionSubscription(for reference: String) -> FiniteSignal<DataCollectionAction> {
        FiniteSignal { callback in
            ...
            bag += self.client.subscribe(subscription: GraphQL.DataCollectionSubscription(reference: reference)).onValue({ data in
                if let extraInformation = data.dataCollectionStatusV2.extraInformation?.asSwedishBankIdExtraInfo {
                    callback(.value(.setAuthInformation(
                        authInformation: .init(swedishAutoStartToken: extraInformation.autoStartToken, norwegianWords: nil)
                    )))
                } else if let extraInformation = data.dataCollectionStatusV2.extraInformation?.asNorwegianBankIdExtraInfo {
                    callback(.value(.setAuthInformation(
                        authInformation: .init(swedishAutoStartToken: nil, norwegianWords: extraInformation.norwegianBankIdWords)
                    )))
                }
    ...
    public override func effects(
           ...
           let provider = getState().provider
        {
            ...
            return FiniteSignal { callback in
                ...
                bag += self.dataCollectionSubscription(for: reference).atValue { action in
                ...
                switch market {
                case .se:
                    self.client.perform(mutation: GraphQL.DataCollectionSwedenMutation(
                        reference: reference,
                        provider: provider,
                        personalNumber: personalNumber
                    )).map { _ in DataCollectionAction.setStatus(status: .started) }.onValue { action in
                        callback(.value(action))
                    }
                case .no:
                    self.client.perform(mutation: GraphQL.DataCollectionNorwayMutation(
                        reference: reference,
                        provider: provider,
                        personalNumber: personalNumber
                    )).map { _ in DataCollectionAction.setStatus(status: .started) }.onValue { action in
