...
class OfferState {
    ...
    func checkoutUpdate(quoteId: String, email: String, ssn: String) -> Future<Void> {
        return self.client
            ...
            .flatMap { data in
                guard data.editQuote.asCompleteQuote?.email == email, data.editQuote.asCompleteQuote?.ssn == ssn else {
    ...
    enum SignEvent {
        case swedishBankId(autoStartToken: String, subscription: CoreSignal<Plain, GraphQL.SignStatusSubscription.Data>)
    ...
    func signQuotes(ids: [String]) -> Future<SignEvent> {
        ...
        return client.perform(mutation: GraphQL.SignQuotesMutation(ids: ids)).map { data in
            if data.signQuotes.asAlreadyCompleted != nil {
                ...
                return SignEvent.swedishBankId(autoStartToken: session.autoStartToken ?? "", subscription: subscription)
