...
extension AnimatedSavingsLabel: Viewable {
    func materialize(events _: ViewableEvents) -> (UIView, Disposable) {
        ...
        container.snp.makeConstraints { make in make.top.bottom.leading.trailing.equalToSuperview() }

        let fromLabel = UILabel(
            value: " ",
            style: TextStyle.brand(.title2(color: .primary)).aligned(to: textAlignment)
        )
        fromLabel.transform = CGAffineTransform(translationX: 0, y: -100)
        container.addArrangedSubview(fromLabel)

        bag += from.atOnce().onValue { amount in fromLabel.value = amount?.formattedAmount ?? " " }
