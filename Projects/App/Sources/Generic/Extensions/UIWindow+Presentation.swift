...
extension UIWindow {
  ...
  func present<P: Presentable>(_ presentable: P, options: PresentationOptions = [], animated: Bool) -> P.Result
  where P.Matter: UIViewController {
    ...
    if animated, !UITraitCollection.isCatalyst {
      ...
      for view in viewController.view.subviews { subviews.append(view) }
