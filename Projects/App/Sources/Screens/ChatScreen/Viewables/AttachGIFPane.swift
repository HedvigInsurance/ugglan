...
extension AttachGIFPane: Viewable {
  func materialize(events: ViewableEvents) -> (UIView, Disposable) {
    ...
    let collectionKit = CollectionKit<EmptySection, AttachGIFImage>(table: Table(rows: []), layout: layout)

    collectionKit.view.contentInset = UIEdgeInsets(top: 0, left: 5, bottom: 0, right: 5)
    ...
    searchBarContainer.snp.makeConstraints { make in make.top.equalToSuperview()
      make.left.equalToSuperview().offset(10)
      make.right.equalToSuperview().offset(-10)
    }

    searchBarView.snp.makeConstraints { make in make.top.equalTo(view)
      make.left.equalTo(view).offset(10)
      make.right.equalTo(view).offset(-10)
    }

    view.addSubview(collectionKit.view)
    collectionKit.view.snp.makeConstraints { make in make.top.equalTo(searchBarView.snp.bottom).offset(10)
      make.left.right.equalTo(view)
      make.bottom.equalTo(view.safeAreaLayoutGuide)
    }

    bag += view.add(infoText) { labelView in
      labelView.snp.makeConstraints { make in labelView.textColor = .darkGray
