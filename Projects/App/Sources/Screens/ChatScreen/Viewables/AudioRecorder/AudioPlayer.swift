...
extension AudioPlayer: Viewable {
	func materialize(events _: ViewableEvents) -> (UIView, Disposable) {
		...
		control.backgroundColor = .brand(.primaryButtonBackgroundColor)
		...
		playIconImageView.snp.makeConstraints { make in make.width.height.equalTo(12) }

		let timeStampLabel = UILabel(
			value: "00:00",
			style: TextStyle.brand(.footnote(color: .link)).rightAligned
		)
		contentView.addArrangedSubview(timeStampLabel)

		timeStampLabel.snp.makeConstraints { make in make.width.equalTo(35) }

		func updateShader(audioPlayer: AVAudioPlayer) {
			let shaderWidth =
				contentView.layer.frame.width * CGFloat(audioPlayer.currentTime / audioPlayer.duration)

			shaderView.snp.updateConstraints { make in make.width.equalTo(shaderWidth) }
		}
