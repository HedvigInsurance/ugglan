...
class ChatState {
	...
	func fetch(cachePolicy: CachePolicy = .returnCacheDataAndFetch, hasFetched: @escaping () -> Void = {}) {
		bag += client.fetch(
		...
		).valueSignal.compactMap(on: .concurrentBackground) { data -> [GraphQL.MessageData]? in
		...
		}.atValue { _ in hasFetched() }.filter(predicate: { messages -> Bool in !messages.isEmpty }).atValue {
			messages in if let message = messages.first { self.handleFirstMessage(message: message) }
		}.onValue { messages in
			...
			if cachePolicy == .returnCacheDataAndFetch { self.fetch(cachePolicy: .fetchIgnoringCacheData) }
		}
	}
	...
	@discardableResult func subscribe() -> CoreSignal<Plain.DropReadWrite, GraphQL.MessageData> {
		...
		let signal = client.subscribe(
			...
			queue: DispatchQueue.global(qos: .background)
		).compactMap(on: .concurrentBackground) { $0.message.fragments.messageData }.filter(predicate: {
			message -> Bool in if self.handledGlobalIds.contains(message.globalId) { return false }
	...
	func sendChatFreeTextResponse(text: String) -> Signal<Void> {
		Signal { callback in let innerBag = DisposeBag()
			...
			innerBag += self.currentMessageSignal.atOnce().take(first: 1).compactMap { $0?.globalId }.take(
				first: 1
			).onValue { globalId in
	...
	init() {
		filteredListSignal = listSignal.atOnce().map(on: .background) { messages in
			messages.enumerated().compactMap { offset, item -> ChatListContent? in
				...
				if item.left?.body == "", !(item.left?.type.isRichType ?? false) { return nil }
		...
		editBag += listSignal.atOnce().onValueDisposePrevious(on: .background) { messages -> Disposable? in
			...
			innerBag += messages.prefix(10).map { message -> Disposable in
				message.left?.onEditCallbacker.addCallback { _ in self.bag.dispose()

					guard
						let firstIndex = self.listSignal.value.firstIndex(where: {
							message -> Bool in message.left?.fromMyself == true
						})
					else { return }

					self.isEditingSignal.value = true

					self.listSignal.value = self.listSignal.value.enumerated().filter {
						offset,
						_ -> Bool in offset > firstIndex
					}.map { $0.1 }

					self.bag += self.client.perform(mutation: GraphQL.EditLastResponseMutation())
