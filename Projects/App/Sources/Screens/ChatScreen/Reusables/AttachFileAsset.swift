...
struct AttachFileAsset: Reusable {
	...
	init(asset: PHAsset, type: AssetType) {
		self.asset = asset
		self.type = type
	}
			...
			{ `self` in let bag = DisposeBag()
				...
				bag += view.signal(for: .touchUpInside).onValue { _ in
					...
					bag += hideOverlayControl.add(loadableButton) { buttonView in
						...
						sendOverlayBag += Signal(after: 0).animated(style: .mediumBounce()) {
							_ in buttonView.transform = CGAffineTransform.identity
						}
					...
					sendOverlayBag += Signal(after: 0).animated(style: .easeOut(duration: 0.25)) {
						_ in effectView.alpha = 1
					}
				...
				PHImageManager.default().requestImage(
					...
					options: nil
				) { image, _ in guard let image = image else { return }
					...
					bag += { imageView.removeFromSuperview() }

					imageView.snp.makeConstraints { make in make.width.height.equalToSuperview() }
