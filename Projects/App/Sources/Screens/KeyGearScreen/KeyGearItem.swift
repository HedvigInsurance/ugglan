...
extension KeyGearItem: Presentable {
	func materialize() -> (UIViewController, Future<Void>) {
		...
		scrollView.snp.makeConstraints { make in make.top.bottom.leading.trailing.equalToSuperview() }
		...
		innerForm.snp.makeConstraints { make in make.top.bottom.trailing.leading.equalToSuperview() }
		...
		bag += receiptFooter.addArranged(
			MultilineLabel(
				value: L10n.keyGearItemViewReceiptTableFooter,
			...
			Future { completion in
				bag += optionsButton.onValue {
					viewController.present(
						Alert(actions: [
							Alert.Action(
								title: L10n.keyGearItemDelete,
								style: .destructive,
								action: { _ in completion(.success) }
							),
							Alert.Action(
				...
				bag += backButton.onValue { _ in completion(.success) }
