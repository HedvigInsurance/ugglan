...
struct KeyGearCategoryButton: SignalProvider {
	...
	var providedSignal: Signal<Void> { callbacker.providedSignal }

	func calculateSize() -> CGSize {
		let attributedString = NSAttributedString(
			styledText: StyledText(text: category.rawValue, style: .brand(.headline(color: .primary)))
		)
...
extension KeyGearCategoryButton: Equatable {
	static func == (lhs: KeyGearCategoryButton, rhs: KeyGearCategoryButton) -> Bool { lhs.category == rhs.category }
}
