...
extension AddKeyGearItem: Presentable {
	func materialize() -> (UIViewController, Future<String>) {
		...
		let cancelButton = UIBarButtonItem(
			title: L10n.keyGearAddItemPageCloseButton,
		...
		bag += state.isValidSignal.atOnce().bindTo(saveButtonContainer, \.isUserInteractionEnabled)
		...
		return (
			...
			Future { completion in
				bag += cancelButton.onValue { completion(.failure(GenericError.cancelled)) }
