...
struct KeyGearImageCarousel { let imagesSignal: ReadSignal<[Either<URL, GraphQL.KeyGearItemCategory>]> }

extension KeyGearImageCarousel: Viewable {
	func materialize(events _: ViewableEvents) -> (UICollectionView, Disposable) {
		let layout = UICollectionViewFlowLayout()
		layout.scrollDirection = .horizontal
		let collectionKit = CollectionKit<EmptySection, KeyGearImageCarouselItem>(
			table: Table(rows: []),
			layout: layout
		)
		...
		collectionKit.view.snp.makeConstraints { make in make.height.equalTo(400) }

		let pagerDots = PagerDots()

		bag += collectionKit.view.add(pagerDots) { pagerDotsView in
			pagerDotsView.snp.makeConstraints { make in make.bottom.equalToSuperview().inset(20) }
		}
