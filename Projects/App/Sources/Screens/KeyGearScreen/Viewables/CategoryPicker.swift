...
struct CategoryPicker { let onSelectCategorySignal: Signal<GraphQL.KeyGearItemCategory> }
...
extension CategoryPicker: Viewable {
    func materialize(events _: ViewableEvents) -> (UIView, Signal<GraphQL.KeyGearItemCategory>) {
        ...
        bag += onSelectCategorySignal.onValue { selectedCategory in
            guard let category = collectionKit.table.first(where: { selectedCategory == $0.category })
            else { return }
        ...
        collectionKit.table = Table(
            rows: GraphQL.KeyGearItemCategory.allCases.map { category in
