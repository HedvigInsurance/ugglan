...
extension KeyGearListCollection: Viewable {
  func materialize(events _: ViewableEvents) -> (UICollectionView, Signal<Effect>) {
    ...
    collectionKit.view.backgroundColor = .brand(.primaryBackground())
    ...
    return (
      collectionKit.view,
      Signal { callback in bag += addButton.onValue { _ in callback(.add) }

        bag += collectionKit.onValue { table in
          bag += table.map { value -> Disposable in
            switch value {
            case let .left(row):
              return row.onValue { _ in callback(.row(id: row.id)) }
            case .right: return NilDisposer()
            }
          }
        }
