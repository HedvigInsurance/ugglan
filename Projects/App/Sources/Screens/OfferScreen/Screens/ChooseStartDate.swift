...
extension ChooseStartDate: Presentable {
	func materialize() -> (UIViewController, Future<Void>) {
		...
		let loadableChooseDateButton = LoadableButton(button: chooseDateButton, initialLoadingState: false)

		let activateNowButton = Button(title: "", type: .transparent(textColor: .brand(.primaryTintColor)))

		let loadableActivateButton = LoadableButton(button: activateNowButton, initialLoadingState: false)
