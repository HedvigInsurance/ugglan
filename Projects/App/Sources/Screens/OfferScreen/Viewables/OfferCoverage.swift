...
extension OfferCoverage: Viewable {
	func materialize(events _: ViewableEvents) -> (UIStackView, Disposable) {
		...
		stackView.addArrangedSubview(
			UILabel(
				value: L10n.offerScreenCoverageTitle,
		...
		stackView.addArrangedSubview(
			UILabel(
				value: L10n.offerScreenInsuredAmountsTitle,
				style: TextStyle.brand(.headline(color: .primary)).centerAligned
			)
		)
		bag += stackView.addArranged(
			ContractInsurableLimits(insurableLimitFragmentsSignal: insurableLimitFragmentsSignal)
		)
