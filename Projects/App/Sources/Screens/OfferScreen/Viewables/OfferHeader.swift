...
struct OfferHeader {
	...
	var onSignTapSignal: Signal<Void> { signCallbacker.providedSignal }
}
...
extension OfferHeader: Viewable {
	func materialize(events _: ViewableEvents) -> (UIView, Disposable) {
		...
		bag += containerScrollView.contentOffsetSignal.onValue { contentOffset in
			view.transform = CGAffineTransform(translationX: 0, y: contentOffset.y / 5)
		}
		...
		let startDateButton = OfferStartDateButton(presentingViewController: presentingViewController)
		...
		bag += signButton.onTapSignal.onValue { _ in self.signCallbacker.callAll() }
