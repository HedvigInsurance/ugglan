...
extension OfferDiscount: Viewable {
	func materialize(events _: ViewableEvents) -> (UIView, Disposable) {
		...
		let redeemButton = Button(
			title: L10n.offerAddDiscountButton,
			type: .outline(borderColor: .clear, textColor: .brand(.primaryText()))
		)

		view.snp.makeConstraints { make in
			make.height.equalTo(
				redeemButton.type.value.height + view.layoutMargins.top + view.layoutMargins.bottom
			)
		}
		...
		func handleButtonState(
			_ buttonView: UIView,
			shouldShowButton: @escaping (_ redeemedCampaigns: [GraphQL.OfferQuery.Data.RedeemedCampaign]) ->
				Bool
		) {
			outState(buttonView)
		...
		bag += view.add(redeemButton) { buttonView in
			handleButtonState(buttonView) { redeemedCampaigns -> Bool in redeemedCampaigns.isEmpty }
		}
		...
		bag += redeemButton.onTapSignal.onValue { _ in let applyDiscount = ApplyDiscount()

			bag += applyDiscount.didRedeemValidCodeSignal.onValue { result in
				self.store.update(
					query: GraphQL.OfferQuery(),
					updater: { (data: inout GraphQL.OfferQuery.Data) in
