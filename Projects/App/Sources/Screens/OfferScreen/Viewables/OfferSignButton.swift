...
struct OfferSignButton {
	...
	var onTapSignal: Signal<Void> { callbacker.providedSignal }
	...
	func materialize(events _: ViewableEvents) -> (UIView, Disposable) {
		...
		bag += scrollView.didLayoutSignal.onValue { _ in
			contentContainer.layoutMargins = UIEdgeInsets(
				top: 10,
				left: 16,
				bottom: self.scrollView.safeAreaInsets.bottom + 10,
				right: 16
			)
		}
		...
		bag += button.onTapSignal.onValue { _ in self.callbacker.callAll() }
