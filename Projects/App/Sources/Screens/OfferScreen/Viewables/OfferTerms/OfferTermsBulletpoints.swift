...
struct OfferTermsBulletPoints { @Inject var client: ApolloClient }
...
extension OfferTermsBulletPoints {
	func bullets(for type: GraphQL.InsuranceType) -> [BulletPoint] {
		...
		if type.isApartment {
			...
			if type.isStudent {
				bulletList.append(
					BulletPoint(title: L10n.offerTermsMaxCompensation(L10n.maxCompensationStudent))
				)
			} else {
				bulletList.append(
					BulletPoint(title: L10n.offerTermsMaxCompensation(L10n.maxCompensation))
				)
			}

			bulletList.append(BulletPoint(title: L10n.offerTermsDeductible(L10n.deductible)))
		} else {
			bulletList.append(BulletPoint(title: L10n.offerHouseTrustHouse))

			bulletList.append(BulletPoint(title: L10n.offerTermsMaxCompensation(L10n.maxCompensationHouse)))
...
extension OfferTermsBulletPoints {
	struct BulletPoint: Viewable {
		...
		func materialize(events _: ViewableEvents) -> (UIStackView, Disposable) {
			...
			checkMark.snp.makeConstraints { make in make.width.equalTo(20) }
