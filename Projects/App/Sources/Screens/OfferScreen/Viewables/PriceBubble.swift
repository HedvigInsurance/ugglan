...
extension PriceBubble: Viewable {
	public func materialize(events _: ViewableEvents) -> (UIView, Disposable) {
		...
		bag += monthlyNetPriceSignal.onValue { values in guard let value = values.last else { return }
			...
			if values.count == 1 { ease.value = CGFloat(value) }
		...
		bag += ease.addSpring(tension: 300, damping: 100, mass: 2) { number in
			if number != 0 {
				let textStyle =
					discountSignal.value > 0
					? TextStyle.brand(.title2(color: .link))
					: TextStyle.brand(.title2(color: .primary))
				priceLabel.styledText = StyledText(text: String(Int(number)), style: textStyle)
			}
		}
		...
		bag += bubbleView.add(campaignBubble) { campaignBubbleView in
			campaignBubbleView.snp.makeConstraints { make in make.right.equalTo(120)
				make.top.equalTo(-10)
			}
		}
