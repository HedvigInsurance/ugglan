...
extension OfferStartDateButton: Viewable {
	func materialize(events _: ViewableEvents) -> (UIStackView, Disposable) {
		...
		button.transform = CGAffineTransform(scaleX: 0.0001, y: 0.0001).concatenating(
		...
		bag += dataSignal.map { $0.lastQuoteOfMember.asCompleteQuote?.startDate?.localDateToDate }.onValue {
			startDay in
