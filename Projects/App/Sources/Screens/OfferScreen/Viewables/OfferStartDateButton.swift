...
extension OfferStartDateButton: Viewable {
	func materialize(events _: ViewableEvents) -> (UIStackView, Disposable) {
		...
		button.transform = CGAffineTransform(scaleX: 0.0001, y: 0.0001)
			.concatenating(CGAffineTransform(translationX: 0, y: -30))
		button.alpha = 0
		bag += button.applyBorderColor { _ -> UIColor in .brand(.primaryText()) }
		...
		bag += dataSignal.take(first: 1)
			.animated(style: SpringAnimationStyle.mediumBounce(delay: 1)) { _ in
		...
		let alert = Alert<Void>(
			title: L10n.alertTitleStartdate,
			message: L10n.alertDescriptionStartdate,
			tintColor: .black,
			actions: [
				Alert.Action(title: L10n.alertCancel, action: {}),
				Alert.Action(
					title: L10n.alertContinue,
					action: {
						self.presentingViewController.present(
							chooseStartDate.wrappedInCloseButton(),
		...
		bag += dataSignal.map { $0.lastQuoteOfMember.asCompleteQuote?.startDate?.localDateToDate }
			.onValue { startDay in
