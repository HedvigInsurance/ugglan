...
struct InsuranceSummarySection {
	...
	init(headerView: UIView? = nil, footerView: UIView? = nil) {
		...
		bag += client.watch(query: GraphQL.MyHomeQuery()).onValueDisposePrevious { data in
			...
			if let isSubleted = insurance.isSubleted {
				...
				isSubletedRow.valueSignal.value =
					isSubleted ? L10n.myHomeRowSubletedValueYes : L10n.myHomeRowSubletedValueNo
			...
			if let extraBuildings = insurance.extraBuildings, !extraBuildings.isEmpty {
				...
				}.map { extraBuildingRow in extraBuildingsSection.append(extraBuildingRow) }
			}
