...
extension InsuranceSummarySection: Viewable {
	func materialize(events _: ViewableEvents) -> (UIView, Disposable) {
		...
		let sectionView = SectionView(headerView: headerView, footerView: footerView)

		stackView.addArrangedSubview(sectionView)

		bag += client.watch(query: GraphQL.MyHomeQuery())
			.onValueDisposePrevious { data in let innerBag = DisposeBag()
				...
				if let yearOfConstruction = insurance.yearOfConstruction {
					...
					yearOfConstructionRow.valueStyleSignal.value = .brand(
						.headline(color: .quartenary)
					)
				...
				if let numberOfBathrooms = insurance.numberOfBathrooms {
					...
					numberOfBathroomsRow.valueStyleSignal.value = .brand(
						.headline(color: .quartenary)
					)
				...
				if let isSubleted = insurance.isSubleted {
					...
					isSubletedRow.valueSignal.value =
						isSubleted
						? L10n.myHomeRowSubletedValueYes : L10n.myHomeRowSubletedValueNo
				...
				if let insuranceType = insurance.type {
					...
					case .house: apartmentTypeRow.valueSignal.value = L10n.myHomeRowTypeHouseValue
				...
				if let extraBuildings = insurance.extraBuildings, !extraBuildings.isEmpty {
					...
					innerBag += { extraBuildingsSection.removeFromSuperview() }

					innerBag +=
						extraBuildings.map { extraBuilding in
							ExtraBuildingRow(
								data: .static(
