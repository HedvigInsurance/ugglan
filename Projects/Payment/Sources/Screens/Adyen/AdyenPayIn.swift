...
extension AdyenMethodsList {
	static var payInOptions: Future<AdyenOptions> {
		...
		return client.fetch(query: GraphQL.AdyenAvailableMethodsQuery()).compactMap { data in
			guard
				let paymentMethodsData = data.availablePaymentMethods.paymentMethodsResponse.data(
					using: .utf8
				),
...
struct AdyenPayIn: Presentable {
	...
	func materialize() -> (UIViewController, Future<Void>) {
		let (viewController, result) = AdyenMethodsList(adyenOptions: adyenOptions) { data, _, onResult in
			...
			Future().delay(by: 0.5).flatMapResult { _ in
