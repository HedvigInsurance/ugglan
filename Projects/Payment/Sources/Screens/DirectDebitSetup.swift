...
extension DirectDebitSetup: Presentable {
    func materialize() -> (UIViewController, Future<Void>) {
        ...
        return (viewController, Future { completion in
            bag += dismissButton.onValue {
                ...
                case .initial:
                    alert = Alert<Bool>.init(
                        ...
                        actions: [
                            Alert.Action(title: L10n.PayInIframeInAppCancelAlert.proceedButton) {
                                true
                            },
                            Alert.Action(title: L10n.PayInIframeInAppCancelAlert.dismissButton) {
                                false
                            },
                        ]
                    )
                case .postOnboarding:
                    alert = Alert<Bool>.init(
                        ...
                        actions: [
                            ...
                            Alert.Action(title: L10n.PayInIframePostSignSkipAlert.dismissButton) {
                                false
                            },
                        ]
                    )
            ...
            bag += NotificationCenter.default.signal(forName: .applicationWillTerminate).onValue { _ in
                self.client.perform(mutation: GraphQL.CancelDirectDebitRequestMutation()).onValue { _ in }
            }
