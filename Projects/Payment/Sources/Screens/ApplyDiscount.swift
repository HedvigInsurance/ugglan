...
public struct ApplyDiscount {
    ...
    private let didRedeemValidCodeCallbacker = Callbacker<GraphQL.RedeemCodeMutation.Data.RedeemCodeV2.AsSuccessfulRedeemResult>()
                ...
                .onValue { _, discountCode in
                    self.client.perform(mutation: GraphQL.RedeemCodeMutation(code: discountCode))
                        ...
                        .onValue { successfulRedeemResult in
                            loadableSubmitButton.isLoadingSignal.value = false
                            self.didRedeemValidCodeCallbacker.callAll(with: successfulRedeemResult)
                            completion(.success)
                        }
                }
