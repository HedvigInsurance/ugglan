...
extension PaymentHeaderCard: Viewable {
    func materialize(events _: ViewableEvents) -> (UIStackView, Disposable) {
        ...
        bag += innerBorderView.applyBorderColor { _ in .brand(.primaryBorderColor) }

        let innerStackView = UIStackView()
        innerStackView.axis = .vertical
        innerBorderView.addSubview(innerStackView)

        innerStackView.snp.makeConstraints { make in make.top.bottom.trailing.leading.equalToSuperview() }
        ...
        topViewStack.snp.makeConstraints { make in make.top.bottom.leading.trailing.equalToSuperview() }

        let leftTopViewStack = UIStackView()
        leftTopViewStack.spacing = 12
        leftTopViewStack.axis = .vertical
        leftTopViewStack.addArrangedSubview(
            UILabel(value: L10n.paymentsCardTitle, style: TextStyle.brand(.subHeadline(color: .tertiary)))
        )

        let dataSignal = client.fetch(query: GraphQL.MyPaymentQuery(
        ...
        bag += leftTopViewStack.addArranged(
            PaymentHeaderPrice(
                grossPriceSignal: grossPriceSignal,
                discountSignal: discountSignal,
                monthlyNetPriceSignal: netSignal
            )
        )
        ...
        bottomViewStack.snp.makeConstraints { make in make.top.bottom.leading.trailing.equalToSuperview() }

        bottomViewStack.addArrangedSubview(
            UILabel(value: L10n.paymentsCardDate, style: .brand(.body(color: .primary)))
        )
