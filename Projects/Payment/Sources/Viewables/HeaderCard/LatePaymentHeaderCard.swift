...
extension LatePaymentHeaderSection: Viewable {
  func materialize(events _: ViewableEvents) -> (UIStackView, Disposable) {
    ...
    bag += childView.applyBorderColor { _ -> UIColor in .brand(.primaryBorderColor) }
    ...
    childView.snp.makeConstraints { make in make.leading.trailing.top.bottom.equalToSuperview() }
    ...
    containerView.snp.makeConstraints { make in make.leading.trailing.equalTo(childView.safeAreaLayoutGuide)
    ...
    icon.snp.makeConstraints { make in make.height.equalTo(25) }
