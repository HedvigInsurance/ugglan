...
extension PaymentHeaderPrice: Viewable {
	func materialize(events _: ViewableEvents) -> (UIView, Disposable) {
		...
		let grossPriceLabel = UILabel(
			value: "",
			style: TextStyle.brand(.title3(color: .tertiary)).restyled { (style: inout TextStyle) in
		...
		bag += combineLatest(discountSignal, grossPriceSignal).animated(
			style: SpringAnimationStyle.mediumBounce(),
			animations: { monthlyDiscount, monthlyGross in grossPriceLabel.value = "\(monthlyGross) kr"
