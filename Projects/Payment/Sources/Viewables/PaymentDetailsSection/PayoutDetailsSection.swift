	...
	extension PayoutDetailsSection: Viewable {
		func materialize(events _: ViewableEvents) -> (SectionView, Disposable) {
			...
			bag += combineLatest(dataSignal, payOutOptions.valueSignal.plain())
				.onValueDisposePrevious { data, options in let bag = DisposeBag()
					...
					if status == .active {
						...
						bag += valueStackView.addArranged(
							MultilineLabel(
								value: L10n.PaymentScreen
									.payOutConnectedPayoutFooterConnected,
						...
						bag += section.append(connectRow)
							.compactMap { connectRow.viewController }.onValue(presentPayOut)
