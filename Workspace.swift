...
let workspace = Workspace(
    ...
    schemes: [
        Scheme(name: "WorkspaceTests",
               shared: true,
               buildAction: nil,
               testAction: TestAction(targets: testableTargets, arguments: Arguments(environment: ["SNAPSHOT_ARTIFACTS": "$(PROJECT_DIR)/Tests/__SnapshotFailures__"], launch: [:])),
               runAction: nil,
               archiveAction: nil),
        Scheme(name: "WorkspaceTests Record",
               shared: true,
               buildAction: nil,
               testAction: TestAction(targets: testableTargets, arguments: Arguments(environment: ["SNAPSHOT_ARTIFACTS": "$(PROJECT_DIR)/Tests/__SnapshotFailures__", "SNAPSHOT_TEST_MODE": "RECORD"], launch: [:])),
               runAction: nil,
               archiveAction: nil),
    ]
)
