...
extension PendingInsurance: Viewable {
    ...
    func materialize(events _: ViewableEvents) -> (UIView, Disposable) {
        ...
        func addBottomContent<G: Viewable>(view: ExpandableRow<G, PendingInsuranceMoreInfo>) {
            ...
            let openButton = Button(title: String(key: .DASHBOARD_PENDING_MORE_INFO), type: .standardSmall(backgroundColor: .hedvig(.lightGray), textColor: .hedvig(.offBlack)))
            let closeButton = Button(title: String(key: .DASHBOARD_PENDING_LESS_INFO), type: .standardSmall(backgroundColor: .hedvig(.lightGray), textColor: .hedvig(.offBlack)))
