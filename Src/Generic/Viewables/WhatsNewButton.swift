...
extension UIViewController { 
    func installWhatsNewButton() {
        let whatsNewButton = WhatsNewButton(presentingViewController: self)
        let item = UIBarButtonItem(viewable: whatsNewButton)
        navigationItem.leftBarButtonItem = item
    }
}

extension WhatsNewButton: Viewable {
    func materialize(events _: ViewableEvents) -> (UIView, Disposable) {
        ...
        bag += chatButtonView.signal(for: .touchUpInside).onValue { _ in
            dashboardOpenWhatsNew(self.presentingViewController)
        }
